...
$(window).on("scroll", function () {
    [...$(".blog-rich-text h2")].forEach((ele) => {
        if (isInViewportCenter(ele)) {
            if (ele.getBoundingClientRect().top >= 0 && ele.getBoundingClientRect().top <= 250) {
...
function scrollFromTop(id) {
    let tocElement = document.querySelector(".sticky-wrapper").style.position;
    ...
    if(tocElement == 'static'){
        finalHeight = finalHeight - 140;
    }
    else if(tocElement == "sticky"){
...
function setTocSticky() {
    let tocElement = document.querySelector(".sticky-wrapper");
    let setHeightTo = tocElement!=null&& tocElement.querySelector(".blog-detail-left")
    ...
    window.addEventListener("scroll", () => {
        if (isInViewportCenter(richtextToTrack)) {
            if (richtextToTrack.getBoundingClientRect().top >= 0 && richtextToTrack.getBoundingClientRect().top <= 200) {
                ...
                gsap.to(tocElement, {position:"sticky", top:"2em", duration:0});
                gsap.to(setHeightTo, {height:'12em', duration:0});
                ...
                gsap.to(tocElement, {position:"static", duration:0})
                gsap.to(setHeightTo, {height:'auto', duration:0})
            }
        }
    })
}